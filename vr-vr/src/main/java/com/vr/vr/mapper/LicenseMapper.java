...
public interface LicenseMapper {
    @Select({"<script>",
        "SELECT id, tenant_id, application_id, granted, quota_type, quota_limit, quota_used, created_at FROM licenses",
    ...
    @Select("SELECT id, tenant_id, application_id, granted, quota_type, quota_limit, quota_used, created_at FROM licenses WHERE id=#{id}")
    License selectLicenseById(Long id);

    @Select("SELECT id, tenant_id, application_id, granted, quota_type, quota_limit, quota_used, created_at FROM licenses WHERE tenant_id=#{tenantId} AND application_id=#{appId}")
    License selectLicenseByTenantAndApp(@Param("tenantId") Long tenantId, @Param("appId") Long appId);

    @Insert("INSERT INTO licenses (tenant_id, application_id, granted, quota_type, quota_limit, quota_used) VALUES (#{tenantId}, #{applicationId}, #{granted}, #{quotaType}, #{quotaLimit}, #{quotaUsed})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insertLicense(License license);

    @Update("UPDATE licenses SET granted=#{granted}, quota_type=#{quotaType}, quota_limit=#{quotaLimit}, quota_used=#{quotaUsed} WHERE id=#{id}")
